...
def expandirNodos(nodoActual=None, nodosExtendidosGlobal=[]):
        ...
        if posicion[0] != 0:#como el siguiente nodo  esta en el limite del tablero
            ...
            if nodoActual.get_padre() is None or nodoActual.get_padre().get_coordenadas() != posicionSiguiente:
                ...
                valorReal = tablero[posicionSiguiente[0]][posicionSiguiente[1]]
...
def costoUniformeRecursivo():
    ...
    nodoActual = A.Arbol(coordenadas=[2,7], valorReal=0, ValorAcumulado=0, valorHijoMenorAcumulado=0)
    nodoMeta = A.Arbol(coordenadas=[2,0], valorReal=1)
    ...
    #for i in range(20):
        ...
        if nodoActual.get_valorHijoMenorAcumulado() == 0:
            ...
            nodoDobleExpancion = expandirNodos(nodoActual= nodoActual, nodosExtendidosGlobal=nodosExtendidosGlobal)
            print("el nodo elejido para hacer la doble expancion fue : {}".format(nodoDobleExpancion))
            nodoActual = nodoDobleExpancion
